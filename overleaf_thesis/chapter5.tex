...
\textbf{Fixed-size chunking} first splits the text at paragraph boundaries (double-newline separators), then groups adjacent paragraphs into chunks of up to 1{,}000 characters with a 200-character overlap between consecutive chunks. The overlap ensures that text near a boundary appears in both adjacent segments, reducing the chance that a relevant clause is cut in half. Paragraphs that on their own exceed the target size are split at an arbitrary character position. The method is fast to compute and exploits only the coarse cue of paragraph breaks, leaving sentence and clause boundaries unrespected.


\textbf{Recursive character chunking} uses the same 1{,}000-character target and 200-character overlap, but tries to cut at natural boundaries before resorting to arbitrary splits: it attempts paragraph breaks first, then single newlines, then sentence boundaries, and only splits mid-text when a segment still exceeds the target size. This produces more coherent chunks than fixed-size splitting wherever the document structure allows.
...
The primary retrieval method is Maximal Marginal Relevance (MMR) search over the ChromaDB index. For each query, MMR first retrieves a candidate pool of four times the number of chunks requested, which gives 40 candidates for $k = 10$, and then selects the final 10 by balancing similarity to the query against diversity among the selected passages. The balance parameter is set to 0.7, weighting the selection toward relevance while still enforcing diversity. The number of retrieved chunks $k$ is configurable from the Streamlit interface. $k=10$ was used for all six configurations in the automated evaluation.|

MMR was chosen because a single contract is internally repetitive: boilerplate definitions, recurring cross-references, and the same phrasing reused across clauses mean that plain similarity search often returns several near-duplicate passages from the very same contract, wasting slots in the fixed retrieval budget on redundant text. The diversity penalty spreads the retrieved passages across different parts of the contract, giving the model broader coverage of the relevant clause and its surrounding context rather than repeated copies of the single strongest match. MMR is used as the retrieval method for all six experimental configurations.
