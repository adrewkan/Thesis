...
Three implementation decisions are compared. The first is how contracts are split into smaller passages before indexing — three methods are tested: fixed-size splitting by character count, recursive splitting that tries to respect sentence and paragraph boundaries, and semantic splitting that groups text by meaning using sentence embeddings. The second decision is which language model generates the answers — Llama 3.1 8B and Mistral 7B are compared, both running locally on a consumer GPU with 4-bit quantization. The third decision is how conversation history is stored — either keeping the last few turns as-is, or compressing the full history into a running summary using the model itself.

These three decisions produce twelve total configurations. All twelve are evaluated on the same set of questions drawn from the CUAD dataset, with the same metrics, to produce a direct comparison.
...
\begin{enumerate}
    ...
    \item Two local language models were integrated and compared, Llama 3.1 8B Instruct and Mistral 7B Instruct, both running on consumer hardware using 4-bit NF4 quantization, with two conversation memory strategies each — windowed and summary — resulting in twelve total system configurations.
    \item All twelve configurations were evaluated on the same stratified sample of CUAD questions using six metrics: Faithfulness, Answer Relevancy, Context Precision, Context Recall, and two LLM-as-a-Judge metrics for answer correctness and conciseness.
...
Related work on language models, retrieval systems, and legal NLP is covered in Chapter 2. Chapter 3 describes the technical background on the core components used in this thesis: large language models, vector databases, chunking methods, and evaluation frameworks. Chapter 4 presents the system architecture and the implementation decisions made at each stage of the pipeline. The experimental setup, including the dataset, the twelve configurations, and the evaluation metrics, is detailed in Chapter 5. Chapter 6 presents the results and compares configurations across all metrics. Chapter 7 concludes the thesis and discusses limitations and possible directions for future work.
